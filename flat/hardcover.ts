...
function mapUserBook(ub: any): Book | null {
	...
	const readDates = (ub.user_book_reads ?? [])
		...
		.filter(Boolean);
	const readAt = readDates.length > 0 ? new Date(readDates[readDates.length - 1]) : null;
...
async function main() {
	...
	const data = await graphql<{ me: Array<{ user_books: any[] }> }>(token, BOOKS_QUERY);

	// Keep original response in data/books
	await Deno.writeTextFile("./data/books/hardcover.json", JSON.stringify(data, null, 2));
	console.log("Saved original response to data/books/hardcover.json");

	const userBooks = data?.me?.[0]?.user_books ?? [];
	const books: Book[] = userBooks.map(mapUserBook).filter((b): b is Book => b != null);

	console.log(`Fetched ${userBooks.length} user_books, ${books.length} after excluding want-to-read`);
	...
	const yamlOpts = { quotingType: '"' as const, forceQuotes: false, replacer: yamlReplacer };

	// current.yaml
	const currentForYaml = prepareBooksForYaml(currentlyReading);
	let currentYaml = yaml.dump({ current: true, books: currentForYaml }, yamlOpts);
